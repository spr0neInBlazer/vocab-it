  ...
  return (
    <>
      ...
      <section className="w-11/12 lg:w-4/5 mx-auto mt-32 mb-6 py-5 px-8 rounded-3xl bg-white text-customText-light dark:text-customText-dark dark:bg-customHighlight flex flex-col">
        {(isEditTitle && title) ? (
          <form className="flex justify-center gap-2 items-center" onSubmit={updateTitle}>
        ...
        <p className="text-lg text-center">{words.length} words</p>
        <div className="my-5">
        ...
        {words.length > 0 ? (
          <section className="w-3/4 mx-auto">
            ...
            <ScrollArea className="h-[210px] rounded-md border px-4 py-3">
              {words.map((w, index) => {
                return (
                  // move key from div to the wrapper
                  <>
        ...
        {isAddWord ? (
          <form className="flex gap-3 my-3 justify-between items-center" onSubmit={addWord}>
            ...
            <input className="w-3/12 text-lg px-2 rounded" value={translation} onChange={(e) => setTranslation(e.target.value)} size={20} autoFocus />
